...
def build_campaign_summary_prompt(df: pd.DataFrame) -> str:
    ...
    return (
        ...
        "ตอบเป็น Markdown เต็มรูปแบบเพื่อให้อ่านง่ายและน่าสนใจ: ใช้ **ตัวหนา** กับชื่อแคมเปญ/แบรนด์/ตัวเลขสำคัญ, "
        "*ตัวเอียง* กับหมายเหตุหรือบริบทเสริม, บล็อกโควท `> ` กับ key takeaway หรือคำเตือนที่สำคัญที่สุด 1 บรรทัด, "
        "และ bullet list (`- `) ตามที่ระบุด้านล่าง ห้ามใช้ header (#) เพราะพื้นที่จำกัด:\n\n"
        "ส่วนที่ 1: ย่อหน้าสั้นๆ 2-3 ประโยค สรุปภาพรวม overall performance ตอนนี้เป็นอย่างไร "
        "ปิดท้ายย่อหน้านี้ด้วย blockquote 1 บรรทัดที่เป็น key takeaway ที่สำคัญที่สุด\n"
